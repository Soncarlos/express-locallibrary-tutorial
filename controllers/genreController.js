...
exports.genre_delete_get = asyncHandler(async (req, res, next) => {
  const [genre, booksHasGenre] = await Promise.all([
    Genre.findById(req.params.id).exec(),
  ...
  if (booksHasGenre) {
    res.render("delete_genre", {
      title:
        booksHasGenre.length > 0
          ? "Can't delete this genre"
          : "Delete this Genre",
      genre: genre,
      GenreinBooks: booksHasGenre,
    });
  }
});
...
exports.genre_update_get = asyncHandler(async (req, res, next) => {
  const [genre, allBooksHasGenre] = await Promise.all([
    Genre.findById(req.params.id).exec(),
  ...
  res.render("genre_form", {
    title: "Update Genre",
    genre: genre,
    allBookshasGenre: allBooksHasGenre,
  });
});

// Handle Genre update on POST.
exports.genre_update_post = [
  body("genre_name", " The name of genre must not be empty")
    .trim()
    .isLength({ min: 2 })
